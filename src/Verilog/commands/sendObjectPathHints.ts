...
export class SendObjectPathHintsCommand extends Command {
  constructor(context: MLIRContext) {
    super("circt-verilog-lsp.objectPathInlayHints", context);
  }
    ...
    for (const editor of vscode.window.visibleTextEditors) {
      ...
      if (mlirFile == "" && designRoot == "") {
        vscode.window.showErrorMessage(
          "No mlir file or design root directory provided. Please provide a mlir file or design root directory in the settings to enable SendObjectPathHints command."
        );
        return;
      }
